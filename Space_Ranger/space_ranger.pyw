...
def try_defeat():
	global defeat
	if c.coords(OBSTACLE)[1] <= c.coords(SHIP)[3] <= c.coords(OBSTACLE)[3] and c.coords(OBSTACLE)[0] <= c.coords(SHIP)[2] <= c.coords(OBSTACLE)[2]:
		c.create_text(WIDTH/2, HEIGHT/7, text = "You crashed\nPress 'Space' to restart", fill = "white", tag = "loser")
		defeat = True

def restart():
	global STAR_X, OBSTACLE_X, ASTEROID_X, defeat, OBSTACLE_SPEED, score
	c.delete("loser")
	score = 0
	STAR_X = random.randint(10, 290)
	ASTEROID_X = random.randint(-20, 240)
...
def move_ship(event):
	...
	if event.keysym == "Left":
		SHIP_SPEED_HOR = -SPEED
	elif event.keysym == "Right":
		SHIP_SPEED_HOR = SPEED
	elif event.keysym == "Up":
		SHIP_SPEED_VERT = -SPEED
